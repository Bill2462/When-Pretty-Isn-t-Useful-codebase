...
class CustomDataset(torch.utils.data.Dataset):
    def __init__(self, path, image_set, transforms):
        
        if image_set == "train":
            image_dir = os.path.join(path, "train", "images")
            mask_dir = os.path.join(path, "train", "masks")
        else:
            image_dir = os.path.join(path, "val", "images")
            mask_dir = os.path.join(path, "val", "masks")

        # Create a list of (image_path, mask_path) tuples
        self.data = []
        for fname in os.listdir(image_dir):
            img_path = os.path.join(image_dir, fname)
            mask_path = os.path.join(mask_dir, fname).replace(".jpg", ".png").replace(".jpeg", ".png").replace(".bmp", ".png")
...
def segmentation_metrics(confmat, eps=1e-6):
    cm = confmat.mat.float()
...
def get_dataset(args, is_train):
    ...
    def custom_ds(*args, **kwargs):
        kwargs.pop("use_v2")
        return CustomDataset(path=args[0], image_set=kwargs["image_set"], transforms=kwargs["transforms"])

    paths = {
        "voc": (args.data_path, voc, 21),
        "voc_aug": (args.data_path, sbd, 21),
        "coco": (args.data_path, get_coco, 21),
        "custom": (args.data_path, custom_ds, 51),
    }
...
def main(args):
    ...
    if args.test_only:
        ...
        metrics = segmentation_metrics(confmat)
        print(
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        try:
            metrics = segmentation_metrics(confmat)
            print(
                f"Epoch {epoch} evaluation metrics: "
                f"Pixel Acc: {metrics['pixel_acc']:.4f}, "
                f"Mean Acc: {metrics['mean_acc']:.4f}, "
                f"mIoU: {metrics['miou']:.4f}, "
                f"FWIoU: {metrics['fwiou']:.4f}, "
            ...
            print(f"Could not compute segmentation metrics due to: {e}")
